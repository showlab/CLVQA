...
def build_bbox_tensors(infos, max_length):
    ...
    # In original annotations, top_left_x, top_left_y, width, height are already normalized.
    for idx, info in enumerate(infos):
        bbox = info["bounding_box"]
        # x = bbox.get("top_left_x", bbox["topLeftX"])
        # y = bbox.get("top_left_y", bbox["topLeftY"])
        if "top_left_x" in bbox.keys() and "top_left_y" in bbox.keys():
            x = bbox.get("top_left_x")
            y = bbox.get("top_left_y")
        elif "topLeftX" in bbox.keys() and "topLeftY" in bbox.keys():
            x = bbox.get("topLeftX")
            y = bbox.get("topLeftY")
        else:
            raise ValueError("Keys for topLeftX and topLeftY not found.")       
