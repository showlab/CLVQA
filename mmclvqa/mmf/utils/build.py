...
def build_multiple_datamodules(
    ...
    for dataset in dataset_list:
        datamodule_instance = build_datamodule(dataset)  # return dataset_builder of this dataset
        ...
        if is_main():
            datamodule_instance.prepare_data(dataset_config)     # prepare_date handles downloads data, annotations, features etc.

        synchronize()
        datamodule_instance.setup(config=dataset_config)                # init trainset, valset, testset for builder()
        if hasattr(datamodule_instance, "update_registry_for_model"):
