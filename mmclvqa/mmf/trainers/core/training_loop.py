...
from mmf.utils.GEMUtils import store_grad, project2cone2, overwrite_grad
        ...
        while self.num_updates < self.max_updates and not should_break:
            ...
            for idx, batch in enumerate(self.train_loader):
                if should_start_update:
                    ...
                    self._start_update()     # ------------------------------------------------------------------------------------------------------------> on update start; optimizer.zero_grad()
                ...
                self._finish_update()       # ------------------------------------------------------------------------------------------------------------> clip gradient if needed; optimizer.step()
                should_start_update = True
...
class CLTrainerTrainingLoopMixin(TrainerTrainingLoopMixin):
    def _finish_update(self):
        if self.training_config.clip_gradients:
            clip_gradients(
                self.model,
        ...
        # TODO
        if self.config.training.CL.use_specific_optim:
            self.scaler.step(self.optimizer, self.model_master.reg_params)
        else:
            self.scaler.step(self.optimizer)
...
class GEMTrainerTrainingLoopMixin(TrainerTrainingLoopMixin):
    def _finish_update(self):
        # modified for GEM, already clip
        # if self.training_config.clip_gradients:
        #     clip_gradients(
        #         self.model,
        #         self.optimizer,
    ...
    def GEMStep(self):
        for prev_task_id, prev_dl in enumerate(self.supp_dls):
            grads_tmp = torch.zeros(sum(self.model_master.grad_dims),).to(self.device)
            for idx, batch in tqdm(enumerate(prev_dl), total=len(prev_dl)):
                self.optimizer.zero_grad()
                p_report = self._forward(batch)
                if self.training_config.exit_on_nan_losses:
                    self._check_nan_losses(p_report)
                loss = extract_loss(p_report, loss_divisor=1)
                p_report.detach()
                self._backward(loss)
                ...
                i = 0
                n_inp = p_report.batch_size
                for param in self.model_master.parameters():
                    if param.grad is not None:
                        beg = 0 if i == 0 else sum(self.model_master.grad_dims[:i])
                        end = sum(self.model_master.grad_dims[:i+1])
                        grads_tmp[beg:end] += param.grad.data.view(-1) * n_inp
                    i += 1
            grads_tmp /= len(prev_dl.get_datasets()[0])
            self.model_master.grads[:, prev_task_id].copy_(grads_tmp)
            self.optimizer.zero_grad()
    
    def GEM_op(self):
        logger.info("===== GEM op ======")
        store_grad(self.model_master.parameters, self.model_master.grads, self.model_master.grad_dims,self.model_master.task_id)
        indx = torch.cuda.LongTensor([i for i in range(self.model_master.task_id)])
        dotp = torch.mm(self.model_master.grads[:, self.model_master.task_id].unsqueeze(0),
                            self.model_master.grads.index_select(1, indx))

        if (dotp < 0).sum() != 0:
            project2cone2(self.model_master.grads[:, self.model_master.task_id].unsqueeze(1),
                              self.model_master.grads.index_select(1, indx), margin=0.5)
                # copy gradients back
            overwrite_grad(self.model_master.parameters,
                               self.model_master.grads[:, self.model_master.task_id],
                               self.model_master.grad_dims)
    
    def run_training_batch(self, batch: Dict[str, Tensor], loss_divisor: int):
        report = self._forward(batch)
        if self.training_config.exit_on_nan_losses:
            self._check_nan_losses(report)
        loss = extract_loss(report, loss_divisor)
