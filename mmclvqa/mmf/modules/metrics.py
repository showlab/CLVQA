...
@registry.register_metric("textvqa_accuracy_cls")
class TextVQAAccuracyCLS(BaseMetric):
    def __init__(self):
        super().__init__("textvqa_accuracy")
        import mmf.utils.m4c_evaluators as evaluators

        self.evaluator = evaluators.ClswiseTextVQAAccuracyEvaluator()
        self.required_params = ["scores", "answers", "context_tokens"]
        self.gt_key = "answers"
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        dict_accuracy = self.evaluator.eval_pred_list(predictions)
        dict_accuracy = {k:torch.tensor(v).to(sample_list.context_tokens.device) for k,v in dict_accuracy.items()}
