...
class BaseProcessor:
    ...
    def __call__(self, item: Any, *args, **kwargs):
        """Main function of the processor. Takes in a dict and returns back a dict
...
class M4CAnswerProcessor(BaseProcessor):
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        self.answer_preprocessor = Processor(config.preprocessor) # simple_word in practice
    ...
    def match_answer_to_vocab_ocr_seq(
        ...
        answer_words = self.tokenize(answer) #"hello world" -> ["hello", "world"] ; "apple" -> "apple"
        answer_word_matches = []
    ...
    def __call__(self, item):
        ...
        scores = torch.zeros(
            self.max_copy_steps, self.get_vocab_size(), dtype=torch.float       #(max_decode_steps, vocab size + max_ocr_token_len) (12, 5000+50=5050)
        )

        # match answers to fixed vocabularies and OCR tokens.
        ocr2inds_dict = defaultdict(list)
        for idx, token in enumerate(item["tokens"]):                               # item['tokens']: [ocr_token1, ocr_token2, ...,<pad>, <pad>...] with max_len_ocr_token_len(50)
            ocr2inds_dict[token].append(idx)
        
        answer_dec_inds = [                                                       # for each answer a, list [(5001,1234,1223)] idx of words in  answer
            self.match_answer_to_vocab_ocr_seq(                                   # return a list, e.g. [(5001,1234,1223)]
