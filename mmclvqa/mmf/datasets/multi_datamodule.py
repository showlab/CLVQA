...
class MultiDataModule(pl.LightningDataModule):
    def __init__(self, config: DictConfig):
        ...
        self.dataset_list: List[str] = dataset_list_from_config(self.config)            # e.g.: multiple: ["GQA", "vqa_v2", "OKVQA"], single: ["vqa_v2", ]
        self.datamodules: Dict[str, pl.LightningDataModule] = build_multiple_datamodules(    # a dict of dataset_builder, E.g.: {"GQA": GQA_builder(), "vqa_v2":vqa_builder()}, builder is subclass of datamodule
    ...
    def _build_multi_dataloader(self, dataset_type: "str" = "train") :#-> MultiDataLoader:
        ...
        for key, datamodule in self.datamodules.items():
            loader_args[key] = getattr(datamodule, f"{dataset_type}_dataloader")()   # e.g. {"GQA": GQA_builder_instance.train_dataloader(),"VQAv2": VQAv2_builder_instance.train_dataloader(),}
