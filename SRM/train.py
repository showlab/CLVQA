...
from utils import *
from dataset import CLVQADataset
from settings import args, TASK_DICT, init_logging, MODEL_CONFIG, MODEL_CLASS, SPECIAL_TOKENS, CONFIG_CLASS
from settings import TOKENIZER, SPECIAL_TOKEN_IDS, FILL_VAL, SAVE_NAME, FINAL_SAVE_NAME, TOKENS_WEIGHT, CONFIG_NAME, GENERATED_SG_PTH
...
def train(task_ids, model):
    tasks = [args.tasks[task_id] for task_id in task_ids] # a list, multi_task: [task1, task2, ...], lll:[task1] (sequentially)

    logger.info("start to train { task: %s, seq train type: %s }" % (tasks, args.seq_train_type))
    model_dir = get_model_dir(tasks)
    make_dir(model_dir)

    train_dataset = [TASK_DICT[args.cl_setting][t]["train"] for t in tasks]
    train_extra_data = []
    if task_ids[0] > 0 and not args.skip_tasks:
        prev_task = args.tasks[task_ids[0]-1]
        with torch.no_grad():
            train_extra_data = fcl_create_data(
                taskList=args.tasks, 
                cur_task=tasks[0], 
                model=model, 
                sg_dta_base_path=GENERATED_SG_PTH[args.cl_setting], 
    ...
    if not model:
        ...
        model = MODEL_CLASS.from_pretrained(args.load_model_name).cuda()
        model.resize_token_embeddings(len(TOKENIZER))
        if not args.fp32:
            model = FP16_Module(model)

    gen_token = get_gen_token(tasks[0], specific=args.add_task_tokens)
    que_token = get_que_token(tasks[0], specific=args.add_task_tokens and args.model_arch=="encoder-decoder")
    TOKENIZER.add_tokens([gen_token, que_token])
    TOKENIZER.save_pretrained(model_dir)
    SPECIAL_TOKENS[tasks[0]] = gen_token
    SPECIAL_TOKENS[f"que_{tasks[0]}"] = que_token
    SPECIAL_TOKEN_IDS[tasks[0]] = TOKENIZER.convert_tokens_to_ids(gen_token)
    SPECIAL_TOKEN_IDS[f"que_{tasks[0]}"] = TOKENIZER.convert_tokens_to_ids(que_token)
    ...
    train_qadata = CLVQADataset(train_dataset, "train", train_extra_data)
    ...
    if "gem" in args.seq_train_type:
        # CLVQA for SRM not yet support for GEM
        raise NotImplementedError

    if args.seq_train_type in REG_TYPE_KEYS:
        # CLVQA for SRM not yet support for MAS or EWC
        raise NotImplementedError
    else:
    ...
    for ep in range(n_train_epochs):
        ...
        for n_steps, batch in enumerate(train_dataloader):
            cqa, Y, gen_X, gen_Y =\
                batch['cqas'], batch['Ys'], batch['gen_Xs'], batch['gen_Ys']
            ...
            losses = get_losses_wo_tkt(parallel_model, cqa, Y, gen_X, gen_Y, train_loss_fct)
            loss = sum(losses)
